...
- Route strictness:
  explicit `--adapter extension` fails closed on invalid or partial package metadata (`EXTENSION_FORMAT_MISMATCH`) and when manifest.json is missing or invalid. Strict extension routing also requires baseline manifest core fields (`manifest_version`, `name`, `version`), requires canonical root-path `manifest.json` presence (nested manifest paths do not satisfy strict routing), requires exactly one canonical root `manifest.json` entry, and requires non-ambiguous extension package entry-path sets (duplicate or case-colliding entry paths fail strict routing). `.crx` inputs require a valid CRX header and embedded ZIP payload in explicit route analysis.
