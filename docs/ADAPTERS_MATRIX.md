...
- Route strictness:
  explicit `--adapter document` fails closed on extension/header mismatch (`.pdf/.rtf/.chm`), requires PDF EOF marker evidence for explicit `.pdf` analysis, and fails closed on invalid/missing or partial OOXML ZIP metadata for `.docm/.xlsm`.
