...
Report card line meanings
- `classification=...`: what was scanned (target/artifact + entry hints).
- `webLane=ACTIVE|NOT_APPLICABLE`: web probe applicability.
- `observed=...`: deterministic counts and bounded signals.
- `posture=...`: current run outcome and top reason.
- `delta=...`: only on CHANGED; numeric movement vs baseline.
