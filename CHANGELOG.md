...
- Added `docs/WEB_RUNTIME_INVENTORY_V0.md` (design contract): deterministic `webRuntimeInventory/0` schema, privacy bounds, and baseline/compare policy for runtime script inventory evidence.
...
- Silent auto-refresh success ticks no longer overwrite the status line every cycle; warning/error states are still surfaced.
...
- Launchpad adapter-doctor parsing now honors deterministic headline status lines (`AdapterDoctorStatus`, `AdapterDoctorCode`, `AdapterDoctorLight`) when present to avoid heuristic guessing.
...
- Artifact-meter workflow summary now explicitly states scanner network posture (`none/no phone-home`) and analysis-only execution mode.
...
- Image strict route now scans bounded ISO descriptor chains for both PVD and descriptor-set terminator evidence (not fixed to only sectors 16/17), allowing valid boot/supplementary descriptor layouts while preserving strict fail-closed checks.
...
- `safe-run`, `run`, `examine`, `email unpack`, `email safe-run`, and `container scan` now also fail closed when the parent of the requested directory-style output path is a file (`*_OUT_PATH_PARENT_NOT_DIRECTORY`).
- `compare`, `ticket-pack`, and `watch` now also fail closed when the parent of the requested directory-style output path is a file (`*_OUT_PATH_PARENT_NOT_DIRECTORY` / `WATCH_OUT_ROOT_PATH_PARENT_NOT_DIRECTORY`).
...
- `safe-run` now fails closed when `--out` overlaps `WEFTEND_ADAPTER_DISABLE_FILE` (`SAFE_RUN_OUT_CONFLICTS_ADAPTER_POLICY_FILE`), preventing staged-finalize cleanup from deleting adapter maintenance policy files.
...
- Report-card viewer summary and adapter-evidence panels now normalize display placeholders to explicit state tokens (`UNKNOWN`, `NOT_REPORTED`, `NOT_APPLICABLE`, `BASELINE_UNAVAILABLE`, `LATEST_UNAVAILABLE`) instead of showing raw `-` placeholders.
