...
const run = (): void => {
  ...
  {
    const tmp = mkTmp();
    const tbzAlias = path.join(tmp, "sample.tbz");
    fs.writeFileSync(tbzAlias, "not-a-real-tbz", "utf8");
    const capture = captureTreeV0(tbzAlias, limits);
    const res = runArtifactAdapterV1({ selection: "archive", enabledPlugins: [], inputPath: tbzAlias, capture });
    assert(!res.ok, "archive tbz alias without plugin should fail closed");
    assertEq(res.failCode, "ARCHIVE_PLUGIN_REQUIRED", "expected plugin required code for tbz alias");
  }
  ...
  {
    const tmp = mkTmp();
    const tbzAlias = path.join(tmp, "bad.tbz");
    fs.writeFileSync(tbzAlias, "not-a-real-tbz", "utf8");
    const capture = captureTreeV0(tbzAlias, limits);
    const res = runArtifactAdapterV1({ selection: "package", enabledPlugins: [], inputPath: tbzAlias, capture });
    assert(!res.ok, "package adapter should fail closed for tbz alias compressed tar package without tar plugin");
    assertEq(res.failCode, "PACKAGE_PLUGIN_REQUIRED", "expected PACKAGE_PLUGIN_REQUIRED for package tbz alias without tar plugin");
  }
  ...
  {
    const tmp = mkTmp();
    const ociDir = path.join(tmp, "oci_layout_capture_partial");
    fs.mkdirSync(path.join(ociDir, "blobs", "sha256"), { recursive: true });
    fs.writeFileSync(path.join(ociDir, "oci-layout"), "{\"imageLayoutVersion\":\"1.0.0\"}\n", "utf8");
    fs.writeFileSync(
      path.join(ociDir, "index.json"),
    ...
    capture.issues = ["CAPTURE_LIMIT_FILES"];
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: ociDir, capture });
    assert(!res.ok, "container adapter should fail closed for explicit container directory when capture evidence is truncated/partial");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for partial capture evidence");
  }
  ...
  {
    const tmp = mkTmp();
    const ociDir = path.join(tmp, "oci_layout_short_digest_ref");
    fs.mkdirSync(path.join(ociDir, "blobs", "sha256"), { recursive: true });
    fs.writeFileSync(path.join(ociDir, "oci-layout"), "{\"imageLayoutVersion\":\"1.0.0\"}\n", "utf8");
    fs.writeFileSync(
      path.join(ociDir, "index.json"),
      JSON.stringify({ schemaVersion: 2, manifests: [{ digest: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" }] }),
      "utf8"
    );
    fs.writeFileSync(path.join(ociDir, "blobs", "sha256", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), "x", "utf8");
    const capture = captureTreeV0(ociDir, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: ociDir, capture });
    assert(!res.ok, "container adapter should fail closed for OCI layout non-canonical digest length");
    assertEq(res.failCode, "CONTAINER_LAYOUT_INVALID", "expected CONTAINER_LAYOUT_INVALID for OCI non-canonical digest length");
  }
  ...
  {
    const tmp = mkTmp();
    const ociDir = path.join(tmp, "oci_layout_duplicate_digest_refs");
    fs.mkdirSync(path.join(ociDir, "blobs", "sha256"), { recursive: true });
    fs.writeFileSync(path.join(ociDir, "oci-layout"), "{\"imageLayoutVersion\":\"1.0.0\"}\n", "utf8");
    fs.writeFileSync(
      path.join(ociDir, "index.json"),
    ...
    const capture = captureTreeV0(ociDir, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: ociDir, capture });
    assert(!res.ok, "container adapter should fail closed for OCI layout duplicate manifest digest references");
    assertEq(res.failCode, "CONTAINER_LAYOUT_INVALID", "expected CONTAINER_LAYOUT_INVALID for OCI layout duplicate digest refs");
  }
...

  {
    ...
    const capture = captureTreeV0(tarPath, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: tarPath, capture });
    assert(!res.ok, "container adapter should fail closed for explicit OCI tar invalid index payload");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for OCI tar invalid index payload");
  }

  {
    const tmp = mkTmp();
    const tarPath = path.join(tmp, "oci_layout_bad_index_shape.tar");
    writeSimpleTar(tarPath, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
    ...
    const capture = captureTreeV0(tarPath, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: tarPath, capture });
    assert(!res.ok, "container adapter should fail closed for explicit OCI tar index manifests shape mismatch");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for OCI tar index manifests shape mismatch");
  }

  {
    const tmp = mkTmp();
    const tarPath = path.join(tmp, "oci_layout_empty_manifests.tar");
    writeSimpleTar(tarPath, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      { name: "index.json", text: "{\"schemaVersion\":2,\"manifests\":[]}\n" },
      { name: "blobs/sha256/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text: "x" },
    ...
    const capture = captureTreeV0(tarPath, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: tarPath, capture });
    assert(!res.ok, "container adapter should fail closed for explicit OCI tar empty manifests");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for OCI tar empty manifests");
  }
  ...
  {
    const tmp = mkTmp();
    const tarPath = path.join(tmp, "oci_layout_short_digest_ref.tar");
    writeSimpleTar(tarPath, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      {
        name: "index.json",
    ...
    const capture = captureTreeV0(tarPath, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: tarPath, capture });
    assert(!res.ok, "container adapter should fail closed for OCI tar non-canonical digest length");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for OCI tar non-canonical digest length");
  }
  ...
  {
    const tmp = mkTmp();
    const tarPath = path.join(tmp, "oci_layout_duplicate_digest_refs.tar");
    writeSimpleTar(tarPath, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      {
        name: "index.json",
    ...
    const capture = captureTreeV0(tarPath, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: tarPath, capture });
    assert(!res.ok, "container adapter should fail closed for OCI tar duplicate manifest digest references");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for OCI tar duplicate digest refs");
  }
  ...
  {
    const tmp = mkTmp();
    const tarPath = path.join(tmp, "oci_layout_directory_markers.tar");
    writeSimpleTar(tarPath, [
    ...
    const capture = captureTreeV0(tarPath, limits);
    const res = runArtifactAdapterV1({ selection: "container", enabledPlugins: [], inputPath: tarPath, capture });
    assert(!res.ok, "container adapter should fail closed when OCI marker names exist only as non-file tar entries");
    assertEq(res.failCode, "CONTAINER_FORMAT_MISMATCH", "expected CONTAINER_FORMAT_MISMATCH for OCI non-file marker entries");
  }
...

  {
    ...
    const bytes = Buffer.alloc(20 * 2048, 0);
    bytes[16 * 2048] = 0; // boot record descriptor
    Buffer.from("CD001", "ascii").copy(bytes, 16 * 2048 + 1);
    bytes[16 * 2048 + 6] = 1;
    bytes[17 * 2048] = 1; // primary volume descriptor
    Buffer.from("CD001", "ascii").copy(bytes, 17 * 2048 + 1);
    bytes[17 * 2048 + 6] = 1;
    bytes[18 * 2048] = 255; // descriptor set terminator
    Buffer.from("CD001", "ascii").copy(bytes, 18 * 2048 + 1);
    ...
    const capture = captureTreeV0(iso, limits);
    const res = runArtifactAdapterV1({ selection: "image", enabledPlugins: [], inputPath: iso, capture });
    assert(res.ok, "image adapter should accept iso chains with boot descriptor before pvd");
    assertEq(res.summary?.counts.isoPvdPresent, 1, "iso pvd should be detected after boot descriptor");
    assertEq(res.summary?.counts.isoTerminatorPresent, 1, "iso terminator should be detected after boot descriptor");
  }
  ...
  {
    const tmp = mkTmp();
    const repo = path.join(tmp, "pseudo_repo_bounded_loose_ref");
    const gitData = path.join(repo, ".gitdata");
    fs.mkdirSync(path.join(gitData, "refs", "heads"), { recursive: true });
    fs.mkdirSync(repo, { recursive: true });
    fs.writeFileSync(path.join(repo, ".git"), "gitdir: .gitdata\n", "utf8");
    ...
    const capture = captureTreeV0(repo, limits);
    const res = runArtifactAdapterV1({ selection: "scm", enabledPlugins: [], inputPath: repo, capture });
    assert(!res.ok, "scm adapter should fail closed for bounded loose-ref evidence in explicit native fallback route");
    assertEq(res.failCode, "SCM_REF_UNRESOLVED", "expected SCM_REF_UNRESOLVED for bounded loose-ref evidence");
  }
  ...
  {
    const tmp = mkTmp();
    const repo = path.join(tmp, "pseudo_repo_case_colliding_loose_vs_packed_ref");
    const gitData = path.join(repo, ".gitdata");
    fs.mkdirSync(path.join(gitData, "refs", "heads"), { recursive: true });
    fs.mkdirSync(repo, { recursive: true });
    fs.writeFileSync(path.join(repo, ".git"), "gitdir: .gitdata\n", "utf8");
    ...
    assert(
      !res.ok,
      "scm adapter should fail closed for case-colliding merged loose/packed ref metadata in explicit native fallback route"
    );
    assertEq(res.failCode, "SCM_REF_UNRESOLVED", "expected SCM_REF_UNRESOLVED for case-colliding merged loose/packed ref metadata");
  }
