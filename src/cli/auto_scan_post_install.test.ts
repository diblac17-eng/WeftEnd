/* src/cli/auto_scan_post_install.test.ts */
export {};
...
const makeTempDir = () => fs.mkdtempSync(path.join(os.tmpdir(), "weftend-watch-once-"));

const runWatchOnce = async (options: { target: string; outRoot: string }): Promise<number> =>
  new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error("watch once timeout"));
    }, 8000);
...
suite("auto scan post-install trigger runs once", async () => {
  ...
  const exitCode = await runWatchOnce({ target, outRoot: root });
