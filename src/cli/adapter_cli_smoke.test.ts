...
const run = async (): Promise<void> => {
...

  {
    ...
    const res = await runCliCapture(["safe-run", tmp, "--out", outDir, "--adapter", "iac"]);
    assertEq(res.status, 40, "safe-run should fail closed when explicit iac text file-set evidence is bounded");
    assert(res.stderr.includes("IAC_UNSUPPORTED_FORMAT"), "expected IAC_UNSUPPORTED_FORMAT on stderr for bounded explicit iac text file-set evidence");
  }

  {
    ...
    const res = await runCliCapture(["safe-run", tmp, "--out", outDir, "--adapter", "cicd"]);
    assertEq(res.status, 40, "safe-run should fail closed when explicit cicd text file-set evidence is bounded");
    assert(res.stderr.includes("CICD_UNSUPPORTED_FORMAT"), "expected CICD_UNSUPPORTED_FORMAT on stderr for bounded explicit cicd text file-set evidence");
  }
...

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const repo = path.join(tmp, "repo_bounded_packed_refs");
    const gitData = path.join(repo, ".gitdata");
    ...
    fs.writeFileSync(path.join(repo, "file.txt"), "x", "utf8");
    const res = await runCliCapture(["safe-run", repo, "--out", outDir, "--adapter", "scm"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit scm bounded packed-refs native fallback evidence");
    assert(res.stderr.includes("SCM_REF_UNRESOLVED"), "expected SCM_REF_UNRESOLVED on stderr for bounded packed-refs evidence");
  }

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const repo = path.join(tmp, "repo_bounded_loose_ref");
    const gitData = path.join(repo, ".gitdata");
    fs.mkdirSync(path.join(gitData, "refs", "heads"), { recursive: true });
    fs.mkdirSync(repo, { recursive: true });
    fs.writeFileSync(path.join(repo, ".git"), "gitdir: .gitdata\n", "utf8");
    fs.writeFileSync(path.join(gitData, "HEAD"), "ref: refs/heads/main\n", "utf8");
    fs.writeFileSync(
      path.join(gitData, "refs", "heads", "main"),
      `0123456789abcdef0123456789abcdef01234567${" ".repeat(300_000)}`,
      "utf8"
    );
    fs.writeFileSync(path.join(repo, "file.txt"), "x", "utf8");
    const res = await runCliCapture(["safe-run", repo, "--out", outDir, "--adapter", "scm"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit scm bounded loose-ref native fallback evidence");
    assert(res.stderr.includes("SCM_REF_UNRESOLVED"), "expected SCM_REF_UNRESOLVED on stderr for bounded loose-ref evidence");
  }
...

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const repo = path.join(tmp, "repo_duplicate_packed_ref");
    const gitData = path.join(repo, ".gitdata");
    fs.mkdirSync(path.join(gitData, "refs", "heads"), { recursive: true });
    fs.mkdirSync(repo, { recursive: true });
    fs.writeFileSync(path.join(repo, ".git"), "gitdir: .gitdata\n", "utf8");
    ...
    fs.writeFileSync(path.join(repo, "file.txt"), "x", "utf8");
    const res = await runCliCapture(["safe-run", repo, "--out", outDir, "--adapter", "scm"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit scm duplicate packed-refs metadata");
    assert(res.stderr.includes("SCM_REF_UNRESOLVED"), "expected SCM_REF_UNRESOLVED on stderr for duplicate packed-refs metadata");
  }

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const repo = path.join(tmp, "repo_case_colliding_packed_ref");
    const gitData = path.join(repo, ".gitdata");
    fs.mkdirSync(path.join(gitData, "refs", "heads"), { recursive: true });
    fs.mkdirSync(repo, { recursive: true });
    fs.writeFileSync(path.join(repo, ".git"), "gitdir: .gitdata\n", "utf8");
    ...
    fs.writeFileSync(path.join(repo, "file.txt"), "x", "utf8");
    const res = await runCliCapture(["safe-run", repo, "--out", outDir, "--adapter", "scm"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit scm case-colliding packed-refs metadata");
    assert(res.stderr.includes("SCM_REF_UNRESOLVED"), "expected SCM_REF_UNRESOLVED on stderr for case-colliding packed-refs metadata");
  }
...

  {
    ...
    const res = await runCliCapture(["safe-run", traversalVsix, "--out", outDir, "--adapter", "extension"]);
    assertEq(res.status, 40, "safe-run should fail closed when extension ZIP metadata includes traversal-style entry paths");
    assert(res.stderr.includes("EXTENSION_FORMAT_MISMATCH"), "expected EXTENSION_FORMAT_MISMATCH on stderr for extension ZIP traversal-style entry path");
  }

  {
    ...
    const res = await runCliCapture(["safe-run", drivePathVsix, "--out", outDir, "--adapter", "extension"]);
    assertEq(res.status, 40, "safe-run should fail closed when extension ZIP metadata includes drive-style absolute entry paths");
    assert(res.stderr.includes("EXTENSION_FORMAT_MISMATCH"), "expected EXTENSION_FORMAT_MISMATCH on stderr for extension ZIP drive-style absolute entry path");
  }
...

  {
    ...
    fs.writeFileSync(iso, bytes);
    const res = await runCliCapture(["safe-run", iso, "--out", outDir, "--adapter", "image"]);
    assertEq(res.status, 0, "safe-run should accept explicit iso descriptor chains with delayed terminator");
  }
...

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociDir = path.join(tmp, "oci_short_digest_ref");
    fs.mkdirSync(path.join(ociDir, "blobs", "sha256"), { recursive: true });
    fs.writeFileSync(path.join(ociDir, "oci-layout"), "{\"imageLayoutVersion\":\"1.0.0\"}\n", "utf8");
    fs.writeFileSync(
      path.join(ociDir, "index.json"),
      "{\"schemaVersion\":2,\"manifests\":[{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}]}\n",
      "utf8"
    );
    fs.writeFileSync(path.join(ociDir, "blobs", "sha256", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), "x", "utf8");
    const res = await runCliCapture(["safe-run", ociDir, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI layout non-canonical digest length");
    assert(res.stderr.includes("CONTAINER_LAYOUT_INVALID"), "expected CONTAINER_LAYOUT_INVALID on stderr for OCI non-canonical digest length");
  }
...

  {
    ...
    fs.writeFileSync(path.join(ociDir, "blobs", "sha256", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), "x", "utf8");
    const res = await runCliCapture(["safe-run", ociDir, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI layout invalid index payload");
    assert(res.stderr.includes("CONTAINER_INDEX_INVALID"), "expected CONTAINER_INDEX_INVALID on stderr for OCI layout invalid index payload");
  }

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociDir = path.join(tmp, "oci_bad_index_shape");
    fs.mkdirSync(path.join(ociDir, "blobs", "sha256"), { recursive: true });
    fs.writeFileSync(path.join(ociDir, "oci-layout"), "{\"imageLayoutVersion\":\"1.0.0\"}\n", "utf8");
    fs.writeFileSync(path.join(ociDir, "index.json"), "{\"schemaVersion\":2,\"manifests\":{}}\n", "utf8");
    fs.writeFileSync(path.join(ociDir, "blobs", "sha256", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), "x", "utf8");
    const res = await runCliCapture(["safe-run", ociDir, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI layout index manifests shape mismatch");
    assert(
      res.stderr.includes("CONTAINER_INDEX_INVALID"),
      "expected CONTAINER_INDEX_INVALID on stderr for OCI layout index manifests shape mismatch"
    );
  }

  {
    ...
    fs.writeFileSync(path.join(ociDir, "blobs", "sha256", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), "x", "utf8");
    const res = await runCliCapture(["safe-run", ociDir, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI layout empty manifests");
    assert(res.stderr.includes("CONTAINER_INDEX_INVALID"), "expected CONTAINER_INDEX_INVALID on stderr for OCI layout empty manifests");
  }

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociDir = path.join(tmp, "oci_duplicate_digest_refs");
    fs.mkdirSync(path.join(ociDir, "blobs", "sha256"), { recursive: true });
    fs.writeFileSync(path.join(ociDir, "oci-layout"), "{\"imageLayoutVersion\":\"1.0.0\"}\n", "utf8");
    fs.writeFileSync(
      path.join(ociDir, "index.json"),
      "{\"schemaVersion\":2,\"manifests\":[{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"},{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}]}\n",
      "utf8"
    );
    fs.writeFileSync(path.join(ociDir, "blobs", "sha256", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), "x", "utf8");
    const res = await runCliCapture(["safe-run", ociDir, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI layout duplicate digest refs");
    assert(res.stderr.includes("CONTAINER_LAYOUT_INVALID"), "expected CONTAINER_LAYOUT_INVALID on stderr for OCI layout duplicate digest refs");
  }
...

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociTarDrivePathMarker = path.join(tmp, "oci_layout_drive_path_marker.tar");
    writeSimpleTar(ociTarDrivePathMarker, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      {
        name: "index.json",
        text: "{\"schemaVersion\":2,\"manifests\":[{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}]}\n",
    ...
    const res = await runCliCapture(["safe-run", ociTarDrivePathMarker, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI tar with drive-style absolute tar entry path");
    assert(res.stderr.includes("CONTAINER_FORMAT_MISMATCH"), "expected CONTAINER_FORMAT_MISMATCH on stderr for OCI tar drive-style absolute entry path");
  }
...

  {
    ...
    writeSimpleTar(ociTarBadIndex, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      { name: "index.json", text: "{ invalid-json" },
      { name: "blobs/sha256/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text: "x" },
    ]);
    const res = await runCliCapture(["safe-run", ociTarBadIndex, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI tar invalid index payload");
    assert(res.stderr.includes("CONTAINER_FORMAT_MISMATCH"), "expected CONTAINER_FORMAT_MISMATCH on stderr for OCI tar invalid index payload");
  }

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociTarBadIndexShape = path.join(tmp, "oci_layout_bad_index_shape.tar");
    writeSimpleTar(ociTarBadIndexShape, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      { name: "index.json", text: "{\"schemaVersion\":2,\"manifests\":{}}\n" },
      { name: "blobs/sha256/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text: "x" },
    ]);
    const res = await runCliCapture(["safe-run", ociTarBadIndexShape, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI tar index manifests shape mismatch");
    assert(res.stderr.includes("CONTAINER_FORMAT_MISMATCH"), "expected CONTAINER_FORMAT_MISMATCH on stderr for OCI tar index manifests shape mismatch");
  }

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociTarEmptyManifests = path.join(tmp, "oci_layout_empty_manifests.tar");
    writeSimpleTar(ociTarEmptyManifests, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      { name: "index.json", text: "{\"schemaVersion\":2,\"manifests\":[]}\n" },
      { name: "blobs/sha256/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text: "x" },
    ]);
    const res = await runCliCapture(["safe-run", ociTarEmptyManifests, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI tar empty manifests");
    assert(res.stderr.includes("CONTAINER_FORMAT_MISMATCH"), "expected CONTAINER_FORMAT_MISMATCH on stderr for OCI tar empty manifests");
  }
...

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociTarShortDigestRef = path.join(tmp, "oci_layout_short_digest_ref.tar");
    writeSimpleTar(ociTarShortDigestRef, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      {
        name: "index.json",
        text: "{\"schemaVersion\":2,\"manifests\":[{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}]}\n",
      },
      { name: "blobs/sha256/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text: "x" },
    ]);
    const res = await runCliCapture(["safe-run", ociTarShortDigestRef, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI tar non-canonical digest length");
    assert(res.stderr.includes("CONTAINER_FORMAT_MISMATCH"), "expected CONTAINER_FORMAT_MISMATCH on stderr for OCI tar non-canonical digest length");
  }
...

  {
    const outDir = mkTmp();
    const tmp = mkTmp();
    const ociTarDuplicateDigestRefs = path.join(tmp, "oci_layout_duplicate_digest_refs.tar");
    writeSimpleTar(ociTarDuplicateDigestRefs, [
      { name: "oci-layout", text: "{\"imageLayoutVersion\":\"1.0.0\"}\n" },
      {
        name: "index.json",
        text: "{\"schemaVersion\":2,\"manifests\":[{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"},{\"digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}]}\n",
      },
      { name: "blobs/sha256/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text: "x" },
    ]);
    const res = await runCliCapture(["safe-run", ociTarDuplicateDigestRefs, "--out", outDir, "--adapter", "container"]);
    assertEq(res.status, 40, "safe-run should fail closed for explicit OCI tar duplicate digest refs");
    assert(res.stderr.includes("CONTAINER_FORMAT_MISMATCH"), "expected CONTAINER_FORMAT_MISMATCH on stderr for OCI tar duplicate digest refs");
  }
